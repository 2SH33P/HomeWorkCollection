...
??? question "我明明没框选项，AI 却把选项识别出来了 / 删了续块还是出现"
    这是模型**凭记忆补全**：如果框住的文字刚好停在「下列叙述错误的是」这类位置，而它又认得这道真题，
    就会把图片里根本没有的选项“背”出来（不是坐标错、也不是缓存）。
    现在已经加**严格约束**（只准输出图片里逐字可见的内容，记得的真题内容不算识别结果，截断处就结束），
    并要求额外校对也不许补。若仍出现：把框调大一点让它包含完整题目，或分两次框（题干一格、选项一格再连成续块），
    然后重新点「AI 识别」。
